...
import { SCHEDULE_DATA, STAFF_CONSTRAINTS, STAFF_LIST, MAIN_ROSTER, HELPER_LIST, LEBARAN_SCHEDULE_DATA } from './constants';
import { DailySchedule } from './types';

export default function App() {
  const [selectedStaff, setSelectedStaff] = useState<string | null>(null);
  const [scheduleType, setScheduleType] = useState<'regular' | 'lebaran'>('regular');
  ...
  return (
    <div className="min-h-screen bg-[#121212] text-white font-sans p-4 md:p-8 selection:bg-[#EE4D2D] selection:text-white">
      <div className="max-w-6xl mx-auto">
        ...
        <header className="mb-8 flex flex-col md:flex-row md:items-center justify-between gap-8">
          ...
          <div className="flex flex-col gap-3">
            {/* Schedule Switcher */}
            <div className="bg-[#1A1A1A] p-1 rounded-2xl border border-slate-800 flex self-end">
              <button 
                onClick={() => setScheduleType('regular')}
                className={`px-4 py-2 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all ${scheduleType === 'regular' ? 'bg-[#EE4D2D] text-white shadow-lg shadow-orange-900/20' : 'text-slate-500 hover:text-slate-300'}`}
              >
                Reguler
              </button>
              <button 
                onClick={() => setScheduleType('lebaran')}
                className={`px-4 py-2 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all ${scheduleType === 'lebaran' ? 'bg-amber-500 text-black shadow-lg shadow-amber-900/20' : 'text-slate-500 hover:text-slate-300'}`}
              >
                Lebaran
              </button>
            </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-4 gap-8">
          ...
          <aside className="lg:col-span-1 space-y-6">
            {/* Lebaran Banner if active */}
            {scheduleType === 'lebaran' && (
              <motion.div 
                initial={{ scale: 0.9, opacity: 0 }}
                animate={{ scale: 1, opacity: 1 }}
                className="bg-gradient-to-br from-amber-400 to-amber-600 rounded-3xl p-6 text-black shadow-2xl border-4 border-amber-300/50"
              >
                <div className="flex items-center gap-2 mb-2">
                  <div className="bg-black text-amber-400 p-1 rounded-lg">
                    <Moon size={16} />
                  </div>
                  <h2 className="font-black uppercase tracking-tighter text-xl italic">Masa Lebaran</h2>
                </div>
                <p className="text-xs font-bold leading-tight opacity-80">Jadwal khusus 7 hari dengan shift tunggal (Sore/Malam).</p>
              </motion.div>
            )}
                ...
                <table className="w-full border-collapse">
                  ...
                  <tbody>
                    {scheduleType === 'regular' ? (
                      ...
                      LEBARAN_SCHEDULE_DATA.map((dayData, idx) => (
                        <motion.tr 
                          key={dayData.day}
                          initial={{ opacity: 0, x: -10 }}
                          animate={{ opacity: 1, x: 0 }}
                          transition={{ delay: idx * 0.05 }}
                          className="border-b border-slate-800/30 hover:bg-amber-500/[0.02] transition-colors"
                        >
                          <td className="p-6">
                            <span className="text-lg font-black italic uppercase tracking-tighter text-amber-500">{dayData.day}</span>
                          </td>
                          <td className="p-6">
                            <div className="flex flex-wrap gap-2">
